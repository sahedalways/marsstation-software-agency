...
const PrivacyPolicyHero = ({ mob }: PolicyHeroProps) => {
    ...
    useEffect(() => {
        ...
        interface Particle {
            ...
            pulseSpeed: number; pulseOffset: number; layer: number;
        }
        interface Connection { from: number; to: number; opacity: number; pulsePhase: number; }
        interface FloatingOrb {
            x: number; y: number; radius: number;
            speedX: number; speedY: number; color1: string; color2: string; phase: number;
        }
        interface EnergyWave {
            centerX: number; centerY: number; radius: number;
            maxRadius: number; opacity: number; color: string; speed: number; active: boolean;
        }
        ...
        const colors = {
            ...
            blue: ['#3b82f6', '#6366f1', '#818cf8', '#60a5fa'],
            cyan: ['#22d3ee', '#06b6d4', '#67e8f9'],
            teal: ['#14b8a6', '#0d9488', '#2dd4bf', '#5eead4'],
        };
        ...
        const initParticles = () => {
            particles = []; connections = []; floatingOrbs = [];
            ...
            for (let i = 0; i < particleCount; i++) {
                ...
                const allColors = [...colors.purple, ...colors.blue, ...colors.teal];
                ...
                particles.push({
                    ...
                    pulseOffset: Math.random() * Math.PI * 2, layer,
                });
            }
            ...
            for (let i = 0; i < particles.length; i++) {
                for (let j = i + 1; j < particles.length; j++) {
                    ...
                    if (dist < maxDist && Math.random() < 0.35) {
                        connections.push({ from: i, to: j, opacity: 1 - dist / maxDist, pulsePhase: Math.random() * Math.PI * 2 });
                    }
                }
            }
            ...
            for (let i = 0; i < (mob ? 2 : 4); i++) {
                floatingOrbs.push({
                    ...
                    color2: colors.teal[Math.floor(Math.random() * colors.teal.length)],
        ...
        const spawnEnergyWave = () => {
            ...
            const waveCols = [...colors.purple, ...colors.teal];
            energyWaves.push({
                centerX: w * 0.5, centerY: h * 0.5,
                radius: mob ? 50 : 120, maxRadius: mob ? 200 : 400,
                opacity: 0.2, color: waveCols[Math.floor(Math.random() * waveCols.length)],
                speed: 0.5 + Math.random() * 0.8, active: true,
            });
        };
        ...
        const draw = (time: number) => {
            ...
            floatingOrbs.forEach((orb) => {
                orb.x += orb.speedX; orb.y += orb.speedY; orb.phase += 0.008;
                if (orb.x < -orb.radius) orb.x = w + orb.radius;
                if (orb.x > w + orb.radius) orb.x = -orb.radius;
                if (orb.y < -orb.radius) orb.y = h + orb.radius;
                if (orb.y > h + orb.radius) orb.y = -orb.radius;
                const breathe = Math.sin(orb.phase) * 0.2 + 0.8;
                const grad = ctx.createRadialGradient(orb.x, orb.y, 0, orb.x, orb.y, orb.radius * breathe);
                grad.addColorStop(0, hexToRgba(orb.color1, 0.06));
                grad.addColorStop(0.6, hexToRgba(orb.color2, 0.02));
                grad.addColorStop(1, 'rgba(0,0,0,0)');
                ctx.fillStyle = grad;
                ctx.fillRect(orb.x - orb.radius * 2, orb.y - orb.radius * 2, orb.radius * 4, orb.radius * 4);
            });
            ...
            energyWaves.forEach((wave) => {
                ...
                wave.opacity *= 0.99;
                if (wave.radius > wave.maxRadius || wave.opacity < 0.01) { wave.active = false; return; }
                ctx.beginPath();
            ...
            connections.forEach((conn) => {
                const a = particles[conn.from], b = particles[conn.to];
            ...
            particles.forEach((p) => {
                p.x = p.baseX + Math.sin(t * 0.4 + p.pulseOffset) * 6 + (mouseX - 0.5) * parallaxStrength * (3 - p.layer);
                p.y = p.baseY + Math.cos(t * 0.3 + p.pulseOffset * 1.2) * 5 + (mouseY - 0.5) * parallaxStrength * (3 - p.layer) * 0.5;
                ...
                ctx.fillStyle = hexToRgba(p.color, p.layer === 0 ? 0.85 : p.layer === 1 ? 0.55 : 0.3);
                ctx.fill();
            });
    ...
    return (
        <>
            <style>{`
                ...
                @keyframes glowPulse {
                    0%, 100% { text-shadow: 0 0 20px rgba(168,85,247,0.4), 0 0 40px rgba(99,102,241,0.2), 0 0 80px rgba(20,184,166,0.15); }
                    50% { text-shadow: 0 0 30px rgba(168,85,247,0.6), 0 0 60px rgba(99,102,241,0.35), 0 0 100px rgba(20,184,166,0.25); }
                }
                .pp-gradient-text {
                    background: linear-gradient(90deg, #a855f7 0%, #6366f1 50%, #14b8a6 100%);
                ...
                .shield-text-overlay {
                    animation: floatText 4s ease-in-out infinite, glowPulse 3s ease-in-out infinite;
                }
            `}</style>
            ...
            <section
                style={{
                    ...
                    background: `
                        ...
                        radial-gradient(ellipse 60% 50% at 30% 80%, rgba(20,184,166,0.12) 0%, transparent 50%),
            ...
            >
                ...
                {/* Shield Icon - Background Decorative Element */}
                <div
                    style={{
                        position: 'absolute',
                        left: '50%',
                ...
                >
                    ...
                    >
                        ...
                        >
                            <path
                                d="M50 5L10 22v28c0 35 17 55 40 65 23-10 40-30 40-65V22L50 5z"
                                fill="url(#shieldGrad)"
                        ...
                        <div
                            style={{
                                ...
                                inset: 0,
                                borderRadius: '50%',
                                background: 'radial-gradient(circle at 50% 50%, rgba(0,0,0,0) 30%, rgba(0,0,0,0.5) 80%)',
                                zIndex: 2,
                            }}
                        />
                        {/* Purple/Teal tint */}
                        <div
                            style={{
                                ...
                                inset: 0,
                                borderRadius: '50%',
                                background: 'radial-gradient(circle at 60% 60%, rgba(147,51,234,0.15) 0%, rgba(20,184,166,0.08) 50%, transparent 80%)',
                                zIndex: 3,
                                mixBlendMode: 'screen',
                            }}
                        />
                ...
                <div
                    style={{
                        ...
                        borderRadius: '50%',
                        border: '1px dashed rgba(20,184,166,0.04)',
                        pointerEvents: 'none',
                        zIndex: 1,
                    }}
                />
                ...
                >
                    ...
                    >
                        {/* Top pill badge */}
                        <motion.div variants={fadeUp} className="pp-pill" style={{ marginBottom: mob ? '14px' : '22px' }}>
                            <span style={{ color: '#14b8a6', fontSize: mob ? '10px' : '12px' }}>🛡</span>
                            <span
                                style={{
                        ...
                        <motion.h1
                            variants={fadeUp}
                            style={{
                                fontSize: mob ? 'clamp(28px, 8vw, 40px)' : 'clamp(48px, 5.5vw, 80px)',
                                fontWeight: 700,
                        ...
                        >
                            Your Privacy
                            <br />
                            <span className="pp-gradient-text">Our Commitment</span>
                        </motion.h1>
                        ...
                        >
                            We value your privacy. Read our policy to understand how we collect, use,
